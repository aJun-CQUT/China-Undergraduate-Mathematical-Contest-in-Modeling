...
class Cattle:
    ...
    def update_metrics(self, x):
        ...
        self.w_years_values = np.append(self.w_years_values, np.sum([self.w_xiaogongniu_values[-1], self.w_xiaomuniu_values[-1], self.w_damuniu_values[-1], self.w_laomuniu_values[-1], w_beta, w_gamma]))
        self.c_years_values = np.append(self.c_years_values, np.sum([self.c_betas_values[-1], self.c_gammas_values[-1], self.c_xiaomunius_values[-1], self.c_damunius_values[-1], self.c_betas_values[-1], self.c_gammas_values[-1], self.c_workers_values[-1], self.m]))
...
if __name__ == '__main__':
    ...
    final_profit = cattle.validate()  # 调用新的验证方法
